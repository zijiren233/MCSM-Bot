...
func Main() {
	...
	base.Parse()
	colorlog.SetLogLevle(colorlog.LogLevle(base.LogLevle))
	go base.Update(version)
	fmt.Printf("%s|\033[97;44m %s \033[0m| 当前日志级别:%s\n", time.Now().Format("[2006-01-02 15:04:05] "), "DEBUG", colorlog.IntToLevle(colorlog.LogLevle(base.LogLevle)))
	// 检查配置文件内是否存在重复ID
	...
	serve := rwmessage.NewServer(gconfig.Mconfig.Cqhttp.Url)
	go serve.Run()
